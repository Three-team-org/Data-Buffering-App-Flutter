...
class DatabaseHelper {
  ...
  Future<List<Map>> getDraft(String date, String user_role, String user_name) async {
    var dbClient = await db;
    List<Map> list = await dbClient.rawQuery('SELECT * FROM calendar_info where date like "$date" AND user_role like "$user_role" AND user_name like "$user_name"');
  ...
  Future<List<Map>> getDraft_admin(String date, String user_role) async {
    var dbClient = await db;
    List<Map> list = await dbClient.rawQuery('SELECT * FROM calendar_info where date like "$date" AND user_role like "$user_role"');
    List<Form_draft> employees = new List();
    print(list.length);
    for (int i = 0; i < list.length; i++) {
      var user =
  ...
  Future<List<Map>> getUserInfo(String user_role) async {
    var dbClient = await db;
    List<Map> list = await dbClient.rawQuery('SELECT * FROM user_info where user_role like "$user_role"');
    ...
    for (int i = 0; i < list.length; i++) {
      ...
      new User_data(list[i]["full_name"],list[i]["doctor_name"],list[i]["dentist_name"], list[i]["birthday"],
          ...
          list[i]["length"], list[i]["time"], list[i]["avatar_path"], list[i]["user_role"]);
      user.setUserId(list[i]["id"]);
  ...
  Future<List<Map>> getHospitalData(String date, String user_role, String user_name) async {
    var dbClient = await db;
    List<Map> list = await dbClient.rawQuery('SELECT * FROM hospital_info where date_save like "$date" AND user_role like "$user_role" AND user_name like "$user_name"');
  ...
  Future<List<Map>> getHospitalData_admin(String date, String user_role) async {
    var dbClient = await db;
    List<Map> list = await dbClient.rawQuery('SELECT * FROM hospital_info where date_save like "$date" AND user_role like "$user_role"');
    List<Hospital_data> employees = new List();
    print(list.length);
    for (int i = 0; i < list.length; i++) {
      var user =
